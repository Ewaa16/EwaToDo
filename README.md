...
| `TZ` | `Asia/Jakarta` di prod agar tanggal "hari ini" benar |
...
3. Set environment variables di Vercel → Project → Settings → Environment Variables: `AUTH_SECRET` (baru), `AUTH_URL=https://<proyek>.vercel.app`, `AUTH_GOOGLE_ID`, `AUTH_GOOGLE_SECRET`, `TURSO_DATABASE_URL`, `TURSO_AUTH_TOKEN`, `TZ=Asia/Jakarta`.
